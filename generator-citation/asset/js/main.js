...
//   récupération des élèments 

let oldCitation = document.getElementById("citation");
let author = document.getElementById("author");
let bouton = document.getElementById("new");

let min = 0;
let max = citations.length ; 

// fontion des citation 
function getItDone() {

  let selectNumber = Math.floor(Math.random() * (max - min) + min);

  oldCitation.textContent = citations[selectNumber][0];
  author.textContent  = citations[selectNumber][1];

}

//  button qui prend le clique 
bouton.addEventListener("click", getItDone);