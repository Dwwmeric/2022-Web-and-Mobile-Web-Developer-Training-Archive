    ...
    public function authenticate(Request $request): PassportInterface
    {
        ...
        return new SelfValidatingPassport(
            new UserBadge($accessToken->getToken(), function() use ($accessToken, $client) {
                ...
                $existingUser = $this->em->getRepository(Utilisateur::class)->findOneBy(['googleId' => $googleUser->getId()]);
                ...
                if ($existingUser) {
					...
					if (!$user) {
						...
						$safeFilename = $this->slugger->slug(basename ($googleUser->getAvatar()));
						$newFilename = $safeFilename.'-'.uniqid().".".Util::urlMimeType($googleUser->getAvatar());
